...
describe('an1t a11y markup contracts', () => {
  ...
  it('An1tSimulator includes 2027 candidate presets (P7-1)', () => {
    const src = readFileSync(join(ROOT, 'components/An1tSimulator.astro'), 'utf8');
    for (const key of [
      'cand-bardella',
      'cand-attal',
      ...
      expect(src).toContain(`data-preset="${key}"`);
      expect(src).toContain(`'${key}':`);
    }
    expect(src).toContain('step="0.1"');
    expect(src).toContain('id="an1t-export-png"');
  });
